...
# Calculate total net profit for each company across the specified years
df['Total Net Profit 2019-2023'] = df[net_profit_cols].sum(axis=1)

# Group by sector and sum the total net profit
sector_net_profit = df.groupby(sector_column)['Total Net Profit 2019-2023'].sum()

# Sort sectors by total net profit in descending order and get the top 3
top_sectors = sector_net_profit.sort_values(ascending=False).head(3)

# Print the result
for sector, total_profit in top_sectors.items():
