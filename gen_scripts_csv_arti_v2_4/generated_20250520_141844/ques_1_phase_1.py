...
# Calculate total net profit per company across the years
df['Total Net Profit 2019-2023'] = df[profit_columns].sum(axis=1)

# Group by sector and sum the total net profit
sector_profit = df.groupby(sector_column)['Total Net Profit 2019-2023'].sum()

# Sort sectors by total net profit in descending order and get the top 3
top_sectors = sector_profit.sort_values(ascending=False).head(3)

# Print the result
for sector, profit in top_sectors.items():
